...
func (_q *AppQuery) QueryUsers() *UserQuery {
...
func (_q *AppQuery) WithUsers(opts ...func(*UserQuery)) *AppQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *AppQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*App, error) {
	...
	if query := _q.withUsers; query != nil {
		if err := _q.loadUsers(ctx, query, nodes,
			func(n *App) { n.Edges.Users = []*User{} },
			func(n *App, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AppQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*App, init func(*App), assign func(*App, *User)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(user.FieldAppID)
	}
