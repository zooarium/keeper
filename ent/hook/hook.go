...
type DivisionFunc func(context.Context, *ent.DivisionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DivisionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DivisionMutation); ok {
		return f(ctx, mv)
	}
