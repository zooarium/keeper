...
func (m *mockRepo) GetByID(ctx context.Context, appID, id int) (*GuestKey, error) {
	for _, k := range m.keys {
		if k.ID == id && (appID == 0 || k.AppID == appID) {
			return k, nil
		}
	}
...
func (m *mockRepo) Update(ctx context.Context, appID, id int, k *GuestKey) (*GuestKey, error) {
	return k, nil
}

func (m *mockRepo) Delete(ctx context.Context, appID, id int) error {
	return nil
}
...
func TestAuthenticateMintsGuestToken(t *testing.T) {
	...
	// Token verifies with the guest secret and carries tenant scope. Guest
	// identity is structural (only guest-secret listeners accept it), not a claim.
