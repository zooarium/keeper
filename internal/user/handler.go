...
func (h *UserHandler) CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	scope, ok := policy.Scope(r.Context(), h.policy, c, "user", "create")
	if !ok {
	...
	if scope == "own" && req.AppID != c.AppID {
		slog.Warn("create user rejected: cross-tenant create not permitted", "user_id", c.UserID, "app_id", c.AppID, "target_app_id", req.AppID)
		render.Error(w, http.StatusForbidden, "access denied")
		return
	}
...
func (h *UserHandler) ListUsers(w http.ResponseWriter, r *http.Request) {
	...
	scope, ok := policy.Scope(r.Context(), h.policy, c, "user", "read")
	if !ok {
		slog.Warn("list users rejected: caller lacks user.read permission", "user_id", c.UserID)
		render.Error(w, http.StatusForbidden, "access denied")
		return
	}
	appID := c.AppID
...
func (h *UserHandler) GetUserByID(w http.ResponseWriter, r *http.Request) {
	...
	scope, ok := policy.Scope(r.Context(), h.policy, c, "user", "read")
	if !ok {
		slog.Warn("get user rejected: caller lacks user.read permission", "id", id, "user_id", c.UserID)
		render.Error(w, http.StatusForbidden, "access denied")
		return
	}
	appID := c.AppID
...
func (h *UserHandler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	scope, ok := policy.Scope(r.Context(), h.policy, c, "user", "update")
	if !ok {
...
func (h *UserHandler) DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	scope, ok := policy.Scope(r.Context(), h.policy, c, "user", "delete")
	if !ok {
		slog.Warn("delete user rejected: caller lacks user.delete permission", "id", id, "user_id", c.UserID)
		render.Error(w, http.StatusForbidden, "access denied")
		return
	}
	appID := c.AppID
