...
// ErrRoleServiceUnavailable is returned when falcon can't be reached (or
// errors) while resolving roles at login. Login fails closed on this error —
// never falls back to a stale/cached role set.
var ErrRoleServiceUnavailable = errors.New("role service unavailable")
...
// NewUserService creates a new user service. roleResolver resolves falcon
// roles at login (see RoleResolver) — login fails closed if it errors.
func NewUserService(repo UserRepository, jwt *auth.JWTManager, roleResolver RoleResolver) UserService {
	return &userService{repo: repo, jwt: jwt, roleResolver: roleResolver}
}
