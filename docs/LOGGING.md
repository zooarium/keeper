...
|---|---|---|---|
| `LOG.DIR` | `KEEPER_LOG_DIR` | `log` | directory for `api.log`; created at startup if missing |
| `LOG.LEVEL` | `KEEPER_LOG_LEVEL` | `info` | `debug` \| `info` \| `warn` \| `error`; unknown values fall back to `info` |
...
logrotate -d /etc/logrotate.d/keeper-api   # dry-run the rotation rule
```
